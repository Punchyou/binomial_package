...
class Binomial(Distribution):
    """ Binomial distribution class for calculating and 
        ...
        Attributes:
            ...
            data_list            (list of floats) a list of floats to be extracted from the data fi    p (float) representing the probability of an event occurring
    ...
    def replace_stats_with_data(self):
        ...
        self.p = len([trial for trial in self.data if trial > 0])/self.n
        self.calculate_mean()
...
# TODO: write a method plot_bar() that outputs a bar chart of the data set according to the following specifications.
    def plot_bar(self):
        ...
        Returns:
            None
        """
        plt.hist(self.data, bins = np.arange(min(self.data), max(self.data) + 1, 1))
        plt.show()
    ...
    def pdf(self, k=0):
        ...
        """
        pdf = binom.pmf(k=k, n=self.n, p=self.p)
        return pdf
